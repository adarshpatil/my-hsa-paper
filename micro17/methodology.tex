\section{Experimental Setup and Methodology} \label{methodology}
...
{\small
\begin{equation*}
H\textnormal{-}MEAN_{CPU} = \frac{n}{\sum_{i=1}^{n} \frac{1}{IPC_{i}^{CPU}}},H\textnormal{-}MEAN_{GPU} = \frac{n}{\sum_{i=1}^{n} \frac{1}{IPC_{i}^{GPU}}} 
\end{equation*}
}
where $IPC_i^{CPU}$ and $IPC_i^{GPU}$ is the instructions per cycle achieved by the $i^{th}$ CPU core and $i^{th}$ GPU CU respectively.
...
\par Combined system performance is measured using the weighted speedup metric \cite{weighted-speedup} which is defined as
{\small
\begin{equation*}
Weighted Speedup = \sum_{i=1}^{n_{cpu}} \frac{IPC_i^{CPU_{IHS}}}{IPC_i^{SP}} + \sum_{i=1}^{n_{cu}} \frac{IPC_i^{GPU_{IHS}}}{IPC_i^{GPU}}
\end{equation*}
}
