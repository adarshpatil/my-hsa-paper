...
Adding a stacked DRAMCache over a IHS architecture is able to recover back just 42\% of its performance when running alone without a DRAMCache. 
...
(i)  the introduction of DRAMCache improves the performance of GPU application by 27\% and 15\% with and without co-running CPU applications with respect to its performance when running alone
(ii) Co-running with CPU applications has only a minor impact on the GPU performance (with or without the DRAM Cache). 
Some cache sensitive GPU workloads show some benefits from the addition of the DRAMCache while other workloads do not significantly reuse cached data thus showing no performance benefits.
