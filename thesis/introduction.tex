...
The primary contribution of this thesis is to improve the performance of IHS architectures using a 3D die stacked DRAMCache by proposing suitable organizations and mechanisms that are heterogeniety aware, improve resource utilization and are efficient and scalable. To summarize, the thesis makes the following contributions:
\begin{itemize}
	\item We quanitify the impact of interference due to co-running of CPU and GPU processors in IHS architectures.
	...
	\item We design an effective DRAMCache organization, \cachename, that is aware of this inherent heterogenity and performance  optimizations accordingly.
	\item We demonstrate using detailed simulation of co-running CPU and GPU workloads that compared to a naive  (heterogenity unaware) DRAMCache, \cachename\ significantly improves the performance of CPU cores abliet with a small reduction in performance of GPU cores.
	\item We show that \cachename\ ourperforms a naive DRAMCache on all IHS system performance metrics.
	\item We compare \cachename\ against state-of-the-art DRAMCache proposals and demonstate that \cachename\ outperforms these schemes.
\end{itemize}
