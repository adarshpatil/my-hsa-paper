...
\end{figure}
The DRAMCache Controller consists of several constituent components as shown in Figure \ref{fig:dramcache-ctrl}. DRAMCache controllers similar to conventional off-chip DRAM controllers consists of a read queue and write queue. Each request in the queue corresponds to the address of a single burst of data from the stacked DRAM device.
In our design, a cache line is stored as a TAD unit and is divided into 2 bursts, where each burst corresponds to the width of the data bus of the stacked DRAM.
However, more subtly unlike conventional DRAM devices each in request in the write queue entails a tag read and match followed by a write to the TAD unit if the tag matches. Thus, both read and write requests 
The highlighted components in Figure \ref{fig:dramcache-ctrl} corresponds to the components that are unique/exclusive to a DRAMCache controller and are not present in a conventional DRAM controller. These supplementary components of the DRAMCache controller - MSHRs, Write Buffers and Fill Queue, incur added complexity to the controller design. An MSHR is allocated on a cache line miss in the DRAMCache and subsequent requests to the cache line are coalesced and added as targets in the MSHR. A single fetch request is sent to memory corresponding to an MSHR. A Write Buffer is allocated on a write miss in the DRAMCache. Since our cache is modelled with a write no-allocate policy, write misses in DRAMCache are forwarded to the memory and a Write Buffer is allocated to ??


Our stacked DRAMCache also faithfully models  a Fill Queue \cite{dca} for fill requests to insert data into the cache on the return path from main memory. We also model MSHR and WriteBuffers with their associated latencies to realize the precise working of caches. To simplify data management and ensure correctness we choose not implement a backing store for the DRAMCache and instead share the backing store instance of the off-chip DRAM itself.
