...
The objectives of \bypassname\ and \prioname\ are, respectively, similar to the Mostly Clean DRAMCache (MCC) \cite{mostly-clean} and Staged Memory Scheduling (SMS) \cite{sms} respectively. A more comprehensive description of the working and mechanisms of SMS and MCC can be found in Section \ref{chap:related-work}. Here we quantitatively compare our work with MCC (adapted for IHS architecture) and SMS.
...
\par Figure \ref{results-comparison} presents performance of CPU and GPU normalized to a IHS with no DRAMCache. 
